...
func (code *simpleQRCode) Generate() []byte {
...
func main() {
	http.HandleFunc("/generate", func(writer http.ResponseWriter, request *http.Request) {
		...
		if err != nil || size == "" {
			fmt.Println("Could not determine the desired QR code size:", err)
			return
		}
